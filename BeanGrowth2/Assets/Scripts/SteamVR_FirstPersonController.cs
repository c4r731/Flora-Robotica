...
public class SteamVR_FirstPersonController : MonoBehaviour
{
	...
	private Valve.VR.EVRButtonId gribButton = Valve.VR.EVRButtonId.k_EButton_Grip;
	private Valve.VR.EVRButtonId menuButton = Valve.VR.EVRButtonId.k_EButton_ApplicationMenu;
    
	private bool gribPressed = false;
	...
	{
		if (device != null) {
			if (device.GetPressUp (gribButton)) {
				mc.PauseGrowth = !mc.PauseGrowth;
			}
			if (device.GetPressDown (menuButton)) {
				bool pauseflag = mc.PauseGrowth;
				Scene scene = SceneManager.GetActiveScene ();
				SceneManager.LoadScene (scene.name);
				mc.PauseGrowth = pauseflag;
			}
		}
	}
